...
describe('Logica di Ripartizione Eccedenze (Selettiva)', () => {
  ...
  it('Fallback: se nessuno è sopra soglia, ripartisce proporzionalmente', () => {
    ...
    const risultato = calcolaRisultati([BERLOCO, NERENXA], bollettaConEccedenzaImpossibile)
    const rigaB = risultato.righe[0]
...
describe('calcolaRisultati - Integrità Totali', () => {
  it('la somma di tutte le voci delle righe deve eguagliare i totali della bolletta', () => {
    ...
    expect(risultato.totali.tariffaAgevolata).toBeCloseTo(BOLLETTA_PDF.tariffaAgevolata, 2)
    expect(risultato.totali.eccedenzaBase).toBeCloseTo(BOLLETTA_PDF.eccedenzaBase, 2)
  ...
  it('paga correttamente il proprietario non residente', () => {
    ...
    expect(rigaNonRes.totaleDaPagare).toBeCloseTo(quotaAttesa + (quotaAttesa * 0.1) + (BOLLETTA_PDF.spesePostali / 2), 2)
  })
})
